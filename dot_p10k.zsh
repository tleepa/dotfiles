# Generated by Powerlevel10k configuration wizard on 2019-11-09 at 20:17 CET.
...
# Toolbox
  if [[ -e /run/.toolboxenv && -r /run/.containerenv ]]; then
    typeset -g my_toolbox_name=${(Q)${${(@M)${(f)"$(</run/.containerenv)"}:#name=*}#name=}}
    function prompt_my_toolbox {
      p10k segment -i $'\uE20F' -b yellow -f black -t "$my_toolbox_name"
    }
  fi

# Distrobox
  if command -v distrobox-export >/dev/null && [[ -r /run/.containerenv ]]; then
    typeset -g my_toolbox_name=${(Q)${${(@M)${(f)"$(</run/.containerenv)"}:#name=*}#name=}}
    function prompt_my_toolbox {
      p10k segment -i $'\uE20F' -b yellow -f black -t "$my_toolbox_name"
    }
